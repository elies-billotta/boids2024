...
void Boid::bounce(float areaSize, const float& size, const float& strength)
{
    ...
    /*for (int i = 0; i < 2; i++)
    {
        if (m_position[i] > areaSize || m_position[i] < -areaSize)
        {
            m_direction[i] *= -1;
        }
    }*/
    ...
    /*for (int i = 0; i < 2; i++)
    {
        if (m_position[i] > areaSize - size)
        {
            boundsForce[i] = -glm::distance(m_position[i], areaSize) * (m_position[i] / areaSize);
        }
        if (m_position[i] < -areaSize + size)
        {
            boundsForce[i] = -glm::distance(m_position[i], -areaSize) * (-areaSize / m_position[i]);
        }
    }*/

    if (m_position.x > areaSize - size)
    {
